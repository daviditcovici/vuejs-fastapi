...
async def read_notes() -> list[NoteOutSchema]:
    return await NoteOutSchema.from_queryset(Notes.all())


async def read_note(note_id) -> NoteOutSchema:
...
async def delete_note(note_id, current_user):
    ...
    if db_note.author.id == current_user.id:
        deleted = await Notes.filter(id=note_id).delete()
        if not deleted:
            raise HTTPException(status_code=400, detail=f"NOT deleted note {note_id}.")
